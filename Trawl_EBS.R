...
training.data<-read.csv("D://EFH Descriptions 2015/Variables/Variables_raw_data/EBS_Trawl_trainingdata.csv",header=TRUE,stringsAsFactors=FALSE)
for(i in 1:length(training.data$btemp)){if(is.na(training.data[i,"btemp"])==TRUE){training.data[i,"btemp"]<-training.data[i,"rtemp"]}}
test.data<-read.csv("D://EFH Descriptions 2015/Variables/Variables_raw_data/EBS_Trawl_testdata.csv",header=TRUE,stringsAsFactors=FALSE)
...
#bathy<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/bathy")
#bathy<-aggregate(bathy, fact=10, fun=mean, expand=TRUE, na.rm=TRUE, filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Bathy")
bathy<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Bathy")
#slope<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/slope")
#slope<-aggregate(slope, fact=10, fun=mean, expand=TRUE, na.rm=TRUE, filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Slope")
slope<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Slope")
#color<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/color")
#color<-aggregate(color, fact=10, fun=mean, expand=TRUE, na.rm=TRUE, filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Color")
color<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Color")
#tmax<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/tmax")
#tmax<-aggregate(color, fact=10, fun=mean, expand=TRUE, na.rm=TRUE, filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Tmax")
tmax<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Tmax")
#bcurrent<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/speed")
#bcurrent<-aggregate(bcurrent, fact=10, fun=mean, expand=TRUE, na.rm=TRUE, filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Bcurrent")
bcurrent<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Bcurrent")
#phi<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/phi")
#phi<-aggregate(phi, fact=10, fun=mean, expand=TRUE, na.rm=TRUE, overwrite=TRUE,filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Phi")
phi<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Phi")
#btemp<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/btemp")
#btemp<-aggregate(btemp, fact=10, fun=mean, expand=TRUE, na.rm=TRUE,overwrite=TRUE,filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Btemp")
btemp<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Btemp")
lat<- init(bathy, v='y')
lat<-mask(lat,bathy,filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Lat",overwrite=TRUE)
lon<- init(bathy, v='x')
lon<-mask(lon,bathy,filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Lon",overwrite=TRUE)
#coral<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/coralfactor")
#coral<-aggregate(coral, fact=10, fun=max, expand=TRUE, na.rm=TRUE,overwrite=TRUE,filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Coralfactor")
coral<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Coralfactor")
#sponge<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/spongefactor")
#sponge<-aggregate(sponge, fact=10, fun=max, expand=TRUE, na.rm=TRUE,overwrite=TRUE,filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Spongefactor")
sponge<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Spongefactor")
#whips<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1ha/whipsfactor")
#whips<-aggregate(whips, fact=10, fun=max, expand=TRUE, na.rm=TRUE,overwrite=TRUE,filename="D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Whipsfactor")
whips<-raster("D://EFH Descriptions 2015/Variables/Variables_EBS_1km/Whipsfactor")

raster.stack<-stack(lon,lat,bathy,slope,btemp,color,bcurrent,phi,tmax, coral, sponge, whips)
names(raster.stack)<-c("lon","lat","bdepth","slope","btemp","color","speed","phi", "tmax", "sponge", "coral", "pen")
...
dir.create(paste("D://EFH Descriptions 2015/Trawl_models/EBS/",species_name[j],sep=""))
results.path <- paste("D://EFH Descriptions 2015/Trawl_models/EBS/",species_name[j],sep="")
...
xvars<-c("s(lon,lat,k=10)","s(slope,k=4)","s(tmax,k=4)","s(color,k=4)","s(phi,k=4)","s(speed,k=4)","s(btemp,k=4)","s(bdepth,k=4)","as.factor(sponge)","as.factor(coral)","as.factor(pen)")
...
pred2 <- predict(cpue.gam, training.dat, fun = predict, na.rm = TRUE, overwrite = TRUE, progress = "text", type = "response", newdata.guaranteed = TRUE)
cpue.obs.pred <- data.frame(cbind(x = yvar, y = pred2))
colnames(cpue.obs.pred)[1]<-"x"

######### Test Data
pred4 <- predict(cpue.gam, test.dat, fun = predict, na.rm = TRUE, overwrite = TRUE, progress = "text", type = "response", newdata.guaranteed = TRUE)
...
for(i in 1:n2){
	...
	if(v1[i]=="phi"){plot(cpue.gam, scale=0, shade=TRUE,select=i,all.terms=TRUE,ylab="Variable effect",xlab="Sediment size (phi)")}
...
png(filename=paste(results.path, "/Figures/CPUEGAMmap.png", sep = ""),width=6,height=7,res=300,units="in")
...
######FIGURE 4 - EFH MAP ##################################################
png(filename=paste(results.path, "/Figures/EFHmap.png", sep = ""),width=6,height=7,res=300,units="in")
par(mfrow=c(1,1),mar=c(5,4,1,1),family="sans")
plot(EFH.raster, main = "",xaxt="n",yaxt="n", box=F,col = colorRampPalette(c("grey","coral3"))(4),ext=EBS.ext.pol,legend=FALSE,ylab="Latitude",xlab="Longitude",horiz=TRUE, zlim = c(2,maxValue(EFH.raster)))
...
writeRaster(predict.CPUE.raster,filename=paste("D://EFH Descriptions 2015/GeoTiffs/GeoTiffs_EBS/",species_name[j], "_CPUE_map.tif", sep = ""),overwrite=TRUE,format="GTiff",options=c("PROFILE=GeoTIFF","TFW=YES"),NAflag=-999)
writeRaster(EFH.raster,filename=paste("D://EFH Descriptions 2015/GeoTiffs/GeoTiffs_EBS/",species_name[j],"_EFHmap.tif", sep = ""),overwrite=TRUE,format="GTiff",options=c("PROFILE=GeoTIFF","TFW=YES"),NAflag=-999)
...
for(i in 1:n2){
	...
	if(v1[i]=="phi"){plot(pa.gam, scale=0, shade=TRUE,select=i,all.terms=TRUE,ylab="Variable effect",xlab="Sediment size (phi)")}
...
png(filename=paste(results.path, "/Figures/PAGAMmap.png", sep = ""),width=6,height=7,res=300,units="in")
par(mfrow=c(1,1),mar=c(5,4,1,1),family="sans")
plot(predict.pa.raster, main = "",xaxt="n",yaxt="n", box=F,col = jet.colors(255),ext=EBS.ext.pol,legend.shrink=0.5,axis.args=list(cex.axis=0.65),legend.args=list(text="Probability of presence",cex=0.65,cex.lab=0.65,side=1,line=2),horiz=TRUE,ylab="Latitude",xlab="Longitude", zlim = c(0,1))
plot(ak.coast, col = "grey80", add = TRUE)
plot(wrld_grd_proj2,lty=3,col="lightgrey",add=T)
axis(1, at=EBS.xaxis.ticks,labels=EBS.xaxis)
axis(2,at=EBS.yaxis.ticks,labels=EBS.yaxis)
...
writeRaster(predict.CPUE.raster,filename=paste("D://EFH Descriptions 2015/GeoTiffs/GeoTiffs_EBS/",species_name[j], "_CPUE_map.tif", sep = ""),overwrite=TRUE,format="GTiff",options=c("PROFILE=GeoTIFF","TFW=YES"),NAflag=-999)
writeRaster(predict.pa.raster,filename=paste("D://EFH Descriptions 2015/GeoTiffs/GeoTiffs_EBS/",species_name[j],"_PA_map.tif", sep = ""),overwrite=TRUE,format="GTiff",options=c("PROFILE=GeoTIFF","TFW=YES"),NAflag=-999)
writeRaster(EFH.raster,filename=paste("D://EFH Descriptions 2015/GeoTiffs/GeoTiffs_EBS/",species_name[j],"_EFH_map.tif", sep = ""),overwrite=TRUE,format="GTiff",options=c("PROFILE=GeoTIFF","TFW=YES"),NAflag=-999)
...
par(mfrow=c(3,3),mar=c(4,4,2,.01))
...
response(maxent.model,var="Grain_size",col=1,rug=FALSE)
dev.off()
...
png(filename=paste(results.path, "/Figures/SuitableHabitatmap.png", sep = ""),width=6,height=7,res=300,units="in")
par(mfrow=c(1,1),mar=c(5,4,1,1),family="sans")
plot(habitat.prediction, main = "",xaxt="n",yaxt="n", box=F,col = jet.colors(255),ext=EBS.ext.pol,legend.shrink=0.5,axis.args=list(cex.axis=0.65),legend.args=list(text="Probability of suitable habitat",cex=0.65,cex.lab=0.65,side=1,line=2),horiz=TRUE,ylab="Latitude",xlab="Longitude", zlim = c(0,1))
plot(ak.coast, col = "grey80", add = TRUE)
plot(wrld_grd_proj2,lty=3,col="lightgrey",add=T)
axis(1, at=EBS.xaxis.ticks,labels=EBS.xaxis)
axis(2,at=EBS.yaxis.ticks,labels=EBS.yaxis)
...
png(filename=paste(results.path, "/Figures/Mappts.png", sep = ""),width=6,height=7,res=300,units="in")
par(mfrow=c(1,1),mar=c(5,4,1,1),family="sans")
plot(maxent.stack[["Depth"]], main = "",xaxt="n",yaxt="n", box=F,ext=EBS.ext.pol,legend.shrink=0.5,axis.args=list(cex.axis=0.65),legend.args=list(text="Depth",cex=0.65,cex.lab=0.65,side=1,line=2),horiz=TRUE,ylab="Latitude",xlab="Longitude")
plot(ak.coast, col = "grey80", add = TRUE)
...
##########################WRITE RASTERS TO ArcGIS FORMAT GeoTIFF's############################################
dir.create(paste(results.path,"/GeoTiffs",sep=""))
writeRaster(habitat.prediction,filename=paste("D://EFH Descriptions 2015/GeoTiffs/GeoTiffs_EBS/",species_name[j],"_SuitableHabitat_map.tif", sep = ""),overwrite=TRUE,format="GTiff",options=c("PROFILE=GeoTIFF","TFW=YES"),NAflag=-999)
writeRaster(EFH.raster,filename=paste("D://EFH Descriptions 2015/GeoTiffs/GeoTiffs_EBS/",species_name[j],"_EFH_map.tif", sep = ""),overwrite=TRUE,format="GTiff",options=c("PROFILE=GeoTIFF","TFW=YES"),NAflag=-999)
...
par(mfrow=c(1,1),mar=c(5,4,1,1),family="sans")
plot(raster.stack[["bdepth"]], main = "",xaxt="n",yaxt="n", box=F,ext=EBS.ext.pol,zlim=c(0,2000),legend.shrink=0.5,axis.args=list(cex.axis=0.65),legend.args=list(text="Depth",cex=0.65,cex.lab=0.65,side=1,line=2),horiz=TRUE,ylab="Latitude",xlab="Longitude")
plot(ak.coast, col = "grey80", add = TRUE)
training.pos<-subset(training.dat,training.dat[species[j]]>0)
test.pos<-subset(test.dat,test.dat[species[j]]>0)
...
legend("bottomleft", legend = legVals, pch = c(20,4),col=c("blue","black"),bty="n", pt.cex = c(2,1) , title = paste("n = ",length(present[,1]),sep=""),cex=1)
...
png(filename=paste("D:/EFH Descriptions 2015/Figures/Figures_EBS/", species_name[j],"_ModelFig.png", sep = ""),width=18,height=7,res=300,units="in")
...
png(filename=paste("D:/EFH Descriptions 2015/Figures/Figures_EBS/", species_name[j],"_ModelFig.png", sep = ""),width=18,height=14,res=300,units="in")
#dev.new(width=18,height=7,units="in",dpi=300)
grid.arrange(grobs=myplot, ncol=3, widths = unit(c(6,6,6),c("in")))
dev.off()
}
...
dev.new(width=6,height=7,res=300,units="in")
par(mfrow=c(1,1),mar=c(5,4,1,1),family="sans")
plot(maxent.stack[["Depth"]], main = "",xaxt="n",yaxt="n", box=F,ext=EBS.ext.pol,zlim=c(0,2000),legend.shrink=0.5,axis.args=list(cex.axis=0.65),legend.args=list(text="Depth",cex=0.65,cex.lab=0.65,side=1,line=2),horiz=TRUE,ylab="Latitude",xlab="Longitude")
plot(ak.coast, col = "grey80", add = TRUE)
points(training.pos,col="blue",pch=20)
points(test.pos,col="purple",pch=20)
plot(wrld_grd_proj2,lty=3,col="lightgrey",add=T)
...
dev.new(width=6,height=7,res=300,units="in")
par(mfrow=c(3,3),mar=c(4,2,2,.01),oma=c(2,4,2,4))
response(maxent.model,var="Depth",range="p",col=1,rug=FALSE)
response(maxent.model,var="Tidal_current",range="p",col=1,rug=FALSE)
response(maxent.model,var="Slope",col=1,range="p",rug=FALSE)
response(maxent.model,var="Color",col=1,range="p",rug=FALSE)
response(maxent.model,var="Temperature",col=1,range="p",rug=FALSE)
response(maxent.model,var="Current_speed",range="p",col=1,rug=FALSE)
response(maxent.model,var="Grain_size",range="p",col=1,rug=FALSE)
